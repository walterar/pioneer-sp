...
local onChat = function (form,ref,option)
	...
	if option == 0 then
		...
		for k,c in ipairs(crewInThisStation) do
			form:AddOption(l.CREWMEMBER_WAGE_PER_WEEK:interp({potentialCrewMember = c.name,wage = showCurrency(c.estimatedWage)}),k)
		end
	end
	...
	local showCandidateDetails = function (response)
		...
		form:AddOption(l.SUGGEST_NEW_WEEKLY_WAGE_OF_N:interp({newAmount=showCurrency(checkOffer(offer+10))}),2)
		...
		form:AddOption(l.SUGGEST_NEW_WEEKLY_WAGE_OF_N:interp({newAmount=showCurrency(checkOffer(offer-10))}),5)
