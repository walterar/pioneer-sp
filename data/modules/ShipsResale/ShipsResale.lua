...
--		form:SetFace({female = false, armour = false, seed = ad.faceseed})
		form:SetMessage(string.interp(l["HelloCommander"..Engine.rand:Integer(1,4)]..l["Sale"..Engine.rand:Integer(1,4)].."[ "..my_ship_name.." ] "..format_num(my_ship_price, 2, "$", "-")))

		for i = 1,maxsales do
			local difer = (saleship[i].basePrice - my_ship_price)
			if difer >= 0 then
				venta = l.His_ship_more..format_num(difer, 2, "$", "-")..l["in_exchange_for_a"..Engine.rand:Integer(1,4)].." [ "..saleship[i].name.." ]"
			else
				venta = l.His_ship_less..format_num(math.abs (difer), 2, "$", "-")..l["in_exchange_for_a"..Engine.rand:Integer(1,4)].." [ "..saleship[i].name.." ]"
			end
