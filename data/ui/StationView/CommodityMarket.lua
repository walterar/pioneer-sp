...
--	price = function (e) return showCurrency(Game.player:GetDockedWith():GetEquipmentPrice(e)) end,
	buy  = function (e) return showCurrency(Game.player:GetDockedWith():GetEquipmentPrice(e)) end,
	sell = function (e) return showCurrency(Game.player:GetDockedWith():GetEquipmentPrice(e)*0.85) end,
...
--	local subone = ui:Button("-1")
	local tradereset = ui:Button("0")--:SetFont("SMALL")
	local addone = ui:Button("1")--:SetFont("SMALL")
	local addten = ui:Button("10")--:SetFont("SMALL")
	local add100 = ui:Button("100")--:SetFont("SMALL")
	local confirmtradebuy = ui:Button(l.CONFIRM_PURCHASE):SetFont("HEADING_NORMAL")--XXX
	local confirmtradesell = ui:Button(l.CONFIRM_SALE):SetFont("HEADING_NORMAL")
	local nobutton = nil
...
--			ui:Margin(16,"LEFT",subone),
			...
			ui:Margin(2,"LEFT",addone),
			ui:Margin(2,"LEFT",addten),
			ui:Margin(2,"LEFT",add100),
		})
	...
	local changeTradeamount = function (delta)
		...
		if trade_mode == trade_mode_buy then
			...
			if stock == 0 then
				buysell:SetInnerWidget(ui:Label(l.NONE_FOR_SALE)
					:SetFont("NORMAL")
...
--			tradetext = "Comprar {amount}t por {price}"
...
--			tradetext = "Vender {amount}t por {price}"
		...
		buysell:SetInnerWidget(ui:Label(string.interp(tradetext,{amount = string.format("%d",tradeamount),price = showCurrency(tradecost)})):SetFont("LARGE"))
	end
...
--	subone.onClick:Connect(function () changeTradeamount(-1) end)
	...
	local doSell = function ()
		local price = Game.player:GetDockedWith():GetEquipmentPrice(tradecommodity)*0.85
		local stock = Game.player:GetDockedWith():GetEquipmentStock(tradecommodity)
		local playerfreecargo = Game.player.freeCapacity
		local orderamount = price * tradeamount
				...
				ui:VBox():PackEnd({
					ui:Margin(16,"VERTICAL", --margin separates the layout from edge above and other widgets below
						ui:HBox():PackEnd({
							...
							ui:Label(marketColumnValue["name"](e)):SetFont("HEADING_NORMAL"), --simple text label with the commodity name
						})
					),
	...
	return
		ui:Grid({58,4,38},1) --make a simple grid layout with 3 columns, 48%, 4% and 48% the width of the space, 1 row (?)
