...
void Sfx::TimeStepUpdate(const float timeStep)
{
	...
	switch (m_type) {
		case TYPE_EXPLOSION: if (m_age > 1.8) m_type = TYPE_NONE;	break;
		case TYPE_DAMAGE:    if (m_age > 1.0) m_type = TYPE_NONE;	break;
		case TYPE_SMOKE:     if (m_age > 0.5) m_type = TYPE_NONE;	break;
...
void SfxManager::Init(Graphics::Renderer *r)
{
	...
	cfg.SetString("explosionFile", "textures/explosions/explosions1.dds");

	cfg.SetString("damagePacking", "billboard,1,1");
	cfg.SetString("smokePacking", "billboard,1,1");
	cfg.SetString("explosionPacking", "atlas,6,36");
	// load
