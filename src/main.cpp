...
start:
	...
	switch (mode) {
		case MODE_GALAXYDUMP: {
			if (argc < 3) {
				Output("pioneersp: galaxy dump requires a filename\n");
				break;
			}
			filename = argv[pos];
			++pos;
			if (argc > pos) { // radius (optional)
				...
				if (end == nullptr || *end != 0 || radius < 0 || radius > 10000) {
					Output("pioneersp: invalid radius: %s\n", argv[pos]);
					break;
				}
				++pos;
			}
			if (argc > pos) { // center of dump (three comma separated coordinates, optional)
				...
				if (end == nullptr || *end != ',' || sx < -10000 || sx > 10000) {
					Output("pioneersp: invalid center: %s\n", argv[pos]);
					break;
				}
		...
		case MODE_USAGE_ERROR:
			Output("pioneersp: unknown mode %s\n", argv[1]);
			// fall through

		case MODE_USAGE:
			Output(
				"usage: pioneersp [mode] [options...]\n"
				"available modes:\n"
				"    -game        [-g]     game (default)\n"
